...
Original file is located at
    https://colab.research.google.com/drive/1fMDeLCPLWn_TLm44tPQZj0L-J87vWZrY
...
def get_pie_plot(feature):
  ...
  plt.title("Relative amount of Area destroyed per "+feature, fontsize = 20)
  plt.show()
...
*   We observe that the most of damage from the forest fires occours during September and August.
*   Apart from these months, majior forest fires occoured during July and March.
...
plt.title("Distribution of area of land affected per month", fontsize = 20)
plt.show()

"""We notice the following from the box plot :

* Majiority of the forest fires occoured during August and September.These include majior forest fires and also very minimal forest fire.Forest fires during this month caused most damage to the forest.

* Chances of a Forest fires in January and November are negligible.

* During May and December there were few forest fires but were quite impactful.

* Few forest fires were experienced March, April, June, October  experienced  but they were mostly calm.This is evident from the mean line of the box plot which in their case lies very close to zero.
...
"""* The data seems evenly distributed altough we see that most of the sundays does not experience forest fires.
* Rate of forest fires on Saturdays and Fridays are also lower than other week days.
* Data seems almost evenely distributed and not much use in it's raw form
...
def numerical_analysis(x ,y, pal="crest", c1="red",c2="green"):
  ...
  ax[2].set_title("Distribution of the  data in various levels of severeity of Forest Fire")
...
"""* For temperature ranging from 5 to 30 the forest fire probability peaks at 22.
...
"""* We can see from the 3rd plot that most dangerous kind of forest fires occour during wind of around 4, while mild forest fires occour at low breeze.
...
"""* As Expected most of the fire occurs when there is no rain.
* We can spot an outlier in the first plot which shows there is a fire even when ther is rain.

# Conclusion

We see that most of the features contain hidden representaions.We are confident that no factor is responsible for such forest fires.We can  include all features for our modelling giving more importance to features like rain, RH, temp etc. and a little less focus on weather invariant features.
"""